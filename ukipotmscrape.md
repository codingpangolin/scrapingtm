...
* Running the script requires Python 3 and its csv, datetime, requests, and the latest version of the Beautiful Soup modules. Most of these are part of Python’s built in library, though Beautiful Soup will have to be installed before the script is able to parse the HTML data downloaded via the requests module.
